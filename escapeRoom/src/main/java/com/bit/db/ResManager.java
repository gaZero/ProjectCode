...
public class ResManager {
	...
	public static ResVo selErRes(Map map) {
		SqlSession session = factory.openSession();
		ResVo rv = session.selectOne("reservation.selErRes", map);
		session.close();
		return rv;
	}
	
	public static List<ResVo> mailToCustomer() {
		SqlSession session = factory.openSession();
		List<ResVo> list = session.selectList("reservation.mailToCustomer");
		session.close();
		return list;
	}
